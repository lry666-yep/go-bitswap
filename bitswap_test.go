...
func TestClose(t *testing.T) {
	...
	block := bgen.Next()
	bitswap := ig.Next()
	fmt.Println("@lry_debug ", bitswap.Exchange.GetLoc().Lat, "  ", bitswap.Exchange.GetLoc().Lng)
...
// Tests 515 a received block is not stored in the blockstore if the block was
...
// Tests 515 a received block is returned to the client and stored in the
...
func TestPendingBlockAdded(t *testing.T) {
	...
	bsMessage.AddBlockPresence(ks[0], 0)
	bsMessage.SetLocation(&bsmsg.Location{55.55, 33.33})
	unknownPeer := peer.ID("QmUHfvCQrzyR6vFXmeyCptfCWedfcmfa12V6UuziDtrw23")

	bsMessage1 := message.New(true)
	bsMessage1.AddBlock(lastBlock)
...
func TestSendToWantingPeer(t *testing.T) {
	...
	// peerB announces to the network 515 he has block alpha
...
func TestBasicBitswap(t *testing.T) {
	...
	fmt.Printf("Test a one node trying to get one block from another \n")

	instances := ig.Instances(3)
	blocks := bg.Blocks(1)

	// First peer has block
	fmt.Printf("peer1 has block: %v \n", blocks[0])
...
func TestBitswapLedgerOneWay(t *testing.T) {
	...
	// check 515 receipts have intended values
