...
// 重载一个NewEngine 传入Location vs 在这直接调用定位获取Location 选择后者
...
// @lry 获取地理位置信息
func (e *Engine) GetLoc() bsmsg.Location {
	return e.loc
}
...
func (e *Engine) nextEnvelope(ctx context.Context) (*Envelope, error) {
	for {
		...
		// 创建回复的message(have/dont_have) 在msg中加入地理位置信息
		msg := bsmsg.New(false)
		var location bsmsg.Location
...
func (e *Engine) MessageReceived(ctx context.Context, p peer.ID, m bsmsg.BitSwapMessage) {
	...
	for _, entry := range wants {
		...
		if !found {
			...
			// 判断请求是want_have还是want_block
			// **是不是可以加个请求类型 want_geo
