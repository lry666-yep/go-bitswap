...
type sessionWantSender struct {
	...
	// @lry666 trace peers distance
	peerGeoTrkr *peerGeoTracker
}
...
// or HAVE / DONT_HAVE
// lry 更新have dont_have 创建change处
...
// UpdateWithGeo is called when the session receives a message with incoming blocks
// @lry重写Update 传入distance location 信息 创建update-change
func (sws *sessionWantSender) UpdateWithGeo(from peer.ID, ks []cid.Cid, haves []cid.Cid, dontHaves []cid.Cid, l bsmsg.Location, d float64) {
	hasUpdate := len(ks) > 0 || len(haves) > 0 || len(dontHaves) > 0
	if !hasUpdate {
		return
	}
...
func (sws *sessionWantSender) onChange(changes []change) {
	...
	// Update wants
	// @lry 在这个函数中将地理位置信息更新到新增参数peerGeoTrkr中
...
func (sws *sessionWantSender) trackWant(c cid.Cid) {
	...
	// @lry wantinfo新增sws.peerGeoTrkr
	wi := newWantInfo(sws.peerRspTrkr, sws.peerGeoTrkr)
...
// It returns all ExhaustedDONT_HAVEs.
// 处理每个peer的update信息
// @lry update中包含have dont_have以及地理位置信息 保存到sws.peerGeoTrkr
func (sws *sessionWantSender) processUpdates(updates []update) []cid.Cid {
	// Process received blocks keys
	blkCids := cid.NewSet()
	// for each peer update 处理接收block的信息
...
// checkForWants checks if there are any wants for which all peers
...
// 有peer可用性变动 更新WantBlockPresence——> 更新bestpeer
...
type wantInfo struct {
	// @lry 跟踪每个peer的地理位置信息, 记录peer与本节点距离
	// geoInfos map[peer.ID]float64 废
	peerGeoTrkr *peerGeoTracker
	...
	// in the session
	// 记录 peer列表每个peer发送过来block的次数，用于最后选择peer
	// 再在这里选出最优节点
	peerRspTrkr *peerResponseTracker
...
// @lry 新增地理位置信息map
func newWantInfo(prt *peerResponseTracker, pgt *peerGeoTracker) *wantInfo {
...
// @lry 新增根据wantInfo中geoInfos更新bestPeer算法
