...
func TestSendWants(t *testing.T) {
	...
	var l bsmsg.Location
	var d float64
	d = 14562.77
	l.Lat = 33.33
	l.Lng = 55.55
	fmt.Println("start")
	spm.UpdateWithGeo(peerA, []cid.Cid{}, []cid.Cid{cids[0]}, []cid.Cid{}, l, d)
	fmt.Println("end")
...
func TestSendsWantBlockToOnePeerOnly(t *testing.T) {
	cids := testutil.GenerateCids(6)
	peers := testutil.GeneratePeers(2)
	peerA := peers[0]
	peerB := peers[1]
	fmt.Printf("peer A = %s, peerB = %s \n", peerA, peerB)
	sid := uint64(1)
	pm := newMockPeerManager()
	fpm := newFakeSessionPeerManager()
	swc := newMockSessionMgr()
	...
	blkCids0 := cids[0:2]
	// blkCids1 := cids[2:4]
	// 本节点添加want信息 want cid0 cid1
	spm.Add(blkCids0)

	// peerA: HAVE cid0
	spm.UpdateWithGeo(peerA, []cid.Cid{}, []cid.Cid{cids[0]}, []cid.Cid{}, bsmsg.Location{55.55, 33.33}, 5000000)
	...
	// peerB: HAVE cid0
	spm.UpdateWithGeo(peerB, []cid.Cid{}, []cid.Cid{cids[0]}, []cid.Cid{}, bsmsg.Location{33.55, 44.33}, 500000)
