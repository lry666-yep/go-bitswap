...
type Session interface {
	exchange.Fetcher
	ID() uint64
	// @lry 实现在session.go中 新实现一个ReceiveFromWithGeo
	ReceiveFrom(peer.ID, []cid.Cid, []cid.Cid, []cid.Cid)
	ReceiveFromWithGeo(peer.ID, []cid.Cid, []cid.Cid, []cid.Cid, bsmsg.Location, float64)
	Shutdown()
}
...
// 从p接收have donthave blks 信息在session中处理
// 重载函数,参数加上地理信息,表示节点p的地理信息 加上RTT信息
...
func (sm *SessionManager) ReceiveFromWithGeo(ctx context.Context, p peer.ID, blks []cid.Cid, haves []cid.Cid, dontHaves []cid.Cid, l bsmsg.Location, d float64) {
	...
	for _, id := range sm.sessionInterestManager.InterestedSessions(blks, haves, dontHaves) {
		...
		if ok {
			sess.ReceiveFromWithGeo(p, blks, haves, dontHaves, l, d)
		}
	}
