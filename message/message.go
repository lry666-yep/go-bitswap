...
type BitSwapMessage interface {
	...
	// the sender.
	// want_have want_block list 发送的want信息
	Wantlist() []Entry

	// Blocks returns a slice of unique blocks.
	Blocks() []blocks.Block
	// BlockPresences returns the list of HAVE / DONT_HAVE in the message
	// CID LIST 标注have / dont_have  返回的have dont_have 信息
...
func (m *impl) Clone() BitSwapMessage {
	...
	msg.locaion.Lat = m.locaion.Lat
	msg.locaion.Lng = m.locaion.Lng
	return msg
}
...
func newMessageFromProto(pbm pb.Message) (BitSwapMessage, error) {
	...
	if pbm.GetLocation() != nil {
		location := new(Location)
		lat, err := strconv.ParseFloat(pbm.GetLocation().GetLat(), 64)
		lng, err := strconv.ParseFloat(pbm.GetLocation().GetLng(), 64)
		if err != nil {
			fmt.Print("error: newMessageFromProto")
		}
		location.Lat = lat
		location.Lng = lng
	...
	// fmt.Println(m.locaion.Lat)
	// fmt.Println(m.locaion.Lng)
...
// @lry 获取&设置Location函数
func (m *impl) Location() Location {
	return m.locaion
}

func (m *impl) SetLocation(location *Location) {
	m.locaion.Lat = location.Lat
	m.locaion.Lng = location.Lng
	// fmt.Println(m.locaion.Lat)
	// fmt.Println(m.locaion.Lng)
}
...
func (m *impl) ToProtoV1() *pb.Message {
	...
	pbm.Location.Lat = strconv.FormatFloat(m.Location().Lat, 'E', 6, 64)
	pbm.Location.Lng = strconv.FormatFloat(m.Location().Lng, 'E', 6, 64)
